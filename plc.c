...
char Address[100];     // PLC IP Address
...
// 设置全局参数，导出到rust时，字符串必须放在最后
void plc_set_param(char* address, int rack, int slot){
    // printf("Set parameters: Address: %s, Rack: %d, Slot: %d.\n", address, rack, slot);
...
int CliConnect()
{
    // printf("Try to connect to plc: Address: %s, Rack: %d, Slot: %d.\n", Address, Rack, Slot);
